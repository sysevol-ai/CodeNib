...
_GRAPH_NAVIGATION_TOOL_IDS = frozenset(
    {"codenib_context", "find_callers", "find_callees", "trace", "lsp_route"}
)
...
class AgentRunner:
    ...
    def _build_default_system_prompt(cls, tools: Sequence[Mapping[str, Any]]) -> str:
        ...
        orient = (
            "Call `codenib_context` first for a compact candidate map."
            if "codenib_context" in names
...
"""
    ...
    def _new_history(self):
        ...
        Returns a :class:`~codenib.agent.history.TokenBudgetedChatHistory`
        when ``max_context_tokens`` is set (so a long loop can't overflow the
        model context), otherwise an unbounded
        :class:`~codenib.agent.history.PlainChatHistory` that behaves like
    ...
    def _apply_input_boundary(self, meta: Any, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        path is untouched. See ``codenib/agent/boundary.py`` (#153).
        """
        line_params = {
            i.name
...
# ---------------------------------------------------------------------------
# Public facade: query() + CodeNibAgentOptions
# ---------------------------------------------------------------------------
#
...
class CodeNibAgentOptions:
    ...
    * ``repo_path`` → ``query()`` calls
      :func:`~codenib.compiler.build_skill_contexts` itself and caches
      indexes under ``index_cache_dir`` (or ``<repo>/.codenib_cache``).
      The "build once at first query" path.
    * ``contexts`` → caller pre-built the contexts dict (advanced; see
      :func:`~codenib.compiler.build_skill_contexts` /
      :func:`~codenib.compiler.load_contexts_from_manifest` for what
      shape to pass).
    * ``manifest`` → caller compiled indexes ahead of time via
      :func:`~codenib.agent.compile_repo` (or
      :class:`~codenib.compiler.IndexCompiler` directly) and passes
      either a loaded :class:`~codenib.compiler.RepoManifest` or a
      ...
      :class:`~codenib.agent.resource_guard.ResourceGuard` can run
...
def _check_exactly_one_mode(opts: CodeNibAgentOptions) -> None:
    ...
    if not set_modes:
        raise ValueError(
            "CodeNibAgentOptions requires exactly one of 'repo_path' "
            "(inline build), 'contexts' (pre-built dict), or 'manifest' "
            "(AoT-compiled RepoManifest or path). All three are unset."
        )
    raise ValueError(
        f"CodeNibAgentOptions requires exactly one of 'repo_path', "
...
def _build_contexts(
    ...
    *,
    ...
    Delegates to ``codenib.compiler.build_skill_contexts`` — the same
