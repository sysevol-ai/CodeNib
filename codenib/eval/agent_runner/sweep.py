...
Given a :class:`~codenib.eval.agent_runner.sweep_config.SweepConfig`, this module
...
def scenario_for(query: str, language_key: str) -> str:
    ...
    from codenib.agent.compile import classify
...
_PRELOAD_RETRIEVER_SKILL = {
    ...
    "bm25": "bm25_search",
    # graph = the codenib_context composer (search seeds + call-graph expand);
    # pulling it into the union builds the "expand" (symbol_graph) context.
    "graph": "codenib_context",
}
    ...
    for arm, skills in cfg.subsets.items():
        ...
        if bad:
            raise ValueError(
                ...
                f"{sorted(defaults)} nor a package under codenib/agent/skills/."
            )
...
def run_cell(
    ...
    *,
    ...
    from codenib.compiler.params import SessionContext
    from codenib.eval.agent_runner.orchestrator import scatter_gather_localize
    from codenib.eval.agent_runner.preload import prepare_preload_query
    ...
    from codenib.eval.agent_runner.verify_expand import run_verify_expand
