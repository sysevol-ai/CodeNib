...
from codenib.eval.agent_runner.symbols import symbol_leaf
from codenib.eval.retrieval_eval import normalize_file_path, parse_answer_spans


def _answer_symbol_names(answer: str) -> List[str]:
    """Names from the agent's ``Symbols:`` line."""
    try:
        from codenib.eval.retrieval_eval import _answer_symbols
...
def load_graph_nav(prebuilt_dir: str, instance_id: str) -> Optional[GraphNav]:
    ...
    if not os.path.exists(path):
        return None
    try:
        from codenib.eval.agent_runner.prebuilt import load_prebuilt_code_graph
