...
"""Analysis policy for the CodeNib Base LSP agent ablation."""
...
def summarize_lsp_agent_study(cells: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    primary_pairs = [
        ...
        if CODENIB_LSP_ARM in pair and LIVE_LSP_ARM in pair
    ]
    return {
        "cell_count": len(cells),
        "primary_pair_count": len(primary_pairs),
        "primary_pair_schema_match_count": sum(
            _pair_config_matches(pair[CODENIB_LSP_ARM], pair[LIVE_LSP_ARM])
...
def analyze_lsp_agent_noninferiority(
    ...
    *,
    ...
    for row in selected:
        ...
        if arm not in {CODENIB_LSP_ARM, LIVE_LSP_ARM}:
            continue
    ...
    for pair in complete:
        static_row = pair[CODENIB_LSP_ARM]
    ...
    return {
        "role": role,
        "estimand": f"{CODENIB_LSP_ARM} - {LIVE_LSP_ARM}",
