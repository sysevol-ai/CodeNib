...
"""Pinned CodeNib Base manifest for the native LSP agent ablation."""
...
from codenib.compiler.snapshot_store import ArtifactProfile, SourceSnapshot
...
def build_base_lsp_agent_manifest(
    ...
    *,
    ...
    """Build a deterministic, leakage-aware manifest from CodeNib Base."""
    ...
    if not subjects:
        raise ValueError("no supported CodeNib Base rows selected")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"codenib-lsp-agent-study-manifest: {exc}\n")
        return 1
