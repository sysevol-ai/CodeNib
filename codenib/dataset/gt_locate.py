...
Usage Examples:
    ...
    python codenib/dataset/gt_locate.py

    # Process all instances using Lite dataset
    python codenib/dataset/gt_locate.py --dataset lite

    # Process first 10 instances of repo "django/django", output into local file
    python codenib/dataset/gt_locate.py --dataset verified \
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output JSON file path (default: ~/.codenib/swebench_{dataset}_gt.json)",
    )
    parser.add_argument(
        "--work-dir",
        type=str,
        default=None,
        help="Working directory for cloning repos (default: ~/.codenib)",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    if args.output is None:
        ...
        args.output = os.path.join(
            str(cache_dir), f"swebench_{args.dataset}_{args.split}_gt.json"
        )
    ...
    default_work_dir = user_state_dir() / "tmp"
    logger.info(f"Repositories cached in: {args.work_dir or default_work_dir}")
    logger.info(f"{'='*60}")

    if args.keep_repos:
        logger.info(f"Keeping all repositories in: {args.work_dir or default_work_dir}")
