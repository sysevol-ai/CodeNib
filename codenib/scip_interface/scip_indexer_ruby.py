    ...
    def _check_indexer_available(self) -> bool:
        ...
        if shutil.which(command[0]) is None:
            logger.error(
                ...
                "scip-cold-start-tools or set CODENIB_RUBY_SCIP_CMD.",
                command[0],
            )
            return False
        return True
    ...
    def _prepare_bundle(self, command: list[str], *, timeout: Optional[int]) -> bool:
        ...
        if bundle_gemfile is None or not bundle_gemfile.exists():
            logger.error(
                ...
                "Set CODENIB_RUBY_BUNDLE_GEMFILE for an overlay Gemfile."
            )
            return False
...
def _has_explicit_bundle_gemfile(env: dict[str, str] | None = None) -> bool:
    ...
    return bool(
        values.get("CODENIB_RUBY_BUNDLE_GEMFILE") or values.get("BUNDLE_GEMFILE")
    )
...
def _ruby_bundle_gemfile(
    ...
    values = env if env is not None else os.environ
    override = values.get("CODENIB_RUBY_BUNDLE_GEMFILE") or values.get("BUNDLE_GEMFILE")
    if override:
