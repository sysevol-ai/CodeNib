...
#
# Evaluate embedding retrieval quality on the CodeNib-base dataset.
#
# Loads pre-built FAISS indices (produced by build_codenib_base_embeddings.sh)
...
#   STORAGE_DIR=${CODENIB_PREBUILT_DIR} RESULTS_DIR=/tmp/eval_results \
#       bash scripts/embeddings/eval_codenib_base_embeddings.sh
#
#   # Force rebuild indices instead of loading pre-built ones
#   bash scripts/embeddings/eval_codenib_base_embeddings.sh --force-rebuild
...
# Keep in sync with build_codenib_base_embeddings.sh
MODELS=(
...
for entry in "${MODELS[@]}"; do
  ...
  MODEL_TAG="${MODEL//\//__}"
  RESULT_FILE="${RESULTS_DIR}/eval_codenib_base_${SPLIT}_${MODEL_TAG}.json"
