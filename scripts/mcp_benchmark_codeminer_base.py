...
indexes under CODEMINER_DATA/<instance_id>. Reports file-level and
...
# index coverage on /mnt/data/codeminer for this dataset, so it's the
...
# language_group (from codeminer-base schema) -> manifest `languages` list.
# Strings on the right match the keys understood by
# codeminer/compiler/index_builders.py.
...
def _make_relative(file_path: str, instance_id: str, repo: str = "") -> str:
    ...
    path that lines up with codeminer-base GT.

    Indexed paths come back with whatever absolute prefix was used at index
    time (e.g. ``/home/<user>/.codeminer/<owner>_<name>/...`` from
    ``CodeMinerBaseDataset.get_repo_path``). GT paths are repo-root-relative
...
async def benchmark_instance(
    ...
    repo_dir = CODEMINER_DATA / instance_id
    ...
    # GT: prefer gt_target_files / gt_code_blocks (codeminer-base schema).
...
async def run_benchmark(
    output_file: str = "results/mcp_codeminer_base.json",
    embedding_model: str = DEFAULT_EMBEDDING_MODEL,
    dimension: int = DEFAULT_DIMENSION,
    limit: int | None = None,
):
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", default="results/mcp_codeminer_base.json")
    p.add_argument(
        "--limit",
        type=int,
        default=None,
