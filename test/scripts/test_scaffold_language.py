...
def test_chunker_only_plan_is_small_and_prints_registry_snippet():
    ...
    assert [str(file.path) for file in files] == [
        "codenib/code_chunking/zig_chunker.py",
    ...
    assert "codenib/code_chunking/__init__.py" in rendered
...
def test_scip_incremental_core_plan_covers_graph_patcher_and_parity_stubs():
    ...
    assert "codenib/scip_interface/scip_indexer_zig.py" in paths
    assert "codenib/scip_interface/scip_decode_zig.py" in paths
    assert "codenib/graph/incremental/patcher_zig.py" in paths
...
def test_lsp_plan_uses_generic_lsp_registry_paths():
    ...
    assert "codenib/ls_index/zig_indexer.py" not in paths
    assert "codenib/ls_index/zig_decode.py" not in paths
    ...
    assert "CODENIB_ZIG_LSP_CMD" in rendered
...
def test_lsp_plan_can_record_scip_cold_start_candidate():
    ...
    assert "codenib/scip_interface/scip_indexer_zig.py" not in paths
    ...
    assert "CODENIB_ZIG_SCIP_CMD" in rendered
...
def test_main_dry_run_prints_without_writing(tmp_path, capsys):
    ...
    assert not (tmp_path / "codenib").exists()
...
def test_main_write_creates_files_and_refuses_overwrite(tmp_path, capsys):
    ...
    assert (tmp_path / "codenib/code_chunking/zig_chunker.py").exists()
