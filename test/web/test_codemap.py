...
def test_derived_file_scan_reuses_generated_paths_within_request(monkeypatch):
    ...
    def generated_paths(repo_dir, commit, pattern):
        grep_calls.append((repo_dir, commit, pattern))
        return frozenset({"generated_a.py", "generated_b.py"})

    monkeypatch.setattr(codemap_module, "git_grep_paths", generated_paths)
    monkeypatch.setattr(
        codemap_module,
        "git_blob_head",
        lambda *_args: b"# Generated by schema compiler. DO NOT EDIT.\n",
    )

    derived = _DerivedFiles("/repo", "a" * 40)

    assert derived("generated_a.py") is True
    assert derived("generated_b.py") is True
    assert len(grep_calls) == 1


def test_historical_codemap_reuses_tree_paths_within_request(tmp_path, monkeypatch):
    tree_calls = []

    def tree_paths(repo_dir, commit):
        tree_calls.append((repo_dir, commit))
        return frozenset({"src/main.py", "src/helper.py"})

    monkeypatch.setattr(codemap_module, "git_tree_paths", tree_paths)
    monkeypatch.setattr(
        codemap_module,
        "git_grep_paths",
    ...
    result = build_codemap(
        _symbol_graph_with_many_call_sites(),
        symbol="caller",
        direction="callees",
        depth=1,
        max_nodes=5,
