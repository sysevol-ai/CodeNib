...
def test_artifact_parser_exposes_only_supported_commands() -> None:
    parser = cli.build_parser()
    top_level = next(
        action
    ...
    artifact = top_level.choices["artifact"]
    commands = next(
        action
        for action in artifact._actions
        if isinstance(action, argparse._SubParsersAction)
    )

    assert sorted(commands.choices) == ["fetch", "mcp-config", "pack", "verify"]
...
def test_mcp_parser_rejects_removed_retained_storage_options(option: str) -> None:
    with pytest.raises(SystemExit) as exc_info:
        cli.build_parser().parse_args(["mcp", option, "retired"])

    assert exc_info.value.code == 2


@pytest.mark.parametrize("option", ("--repo", "--repository"))
def test_mcp_source_constraints_require_an_artifact(option: str) -> None:
    args = cli.build_parser().parse_args(["mcp", option, "/tmp/context"])

    with pytest.raises(cli.CLIError, match="requires --artifact"):
