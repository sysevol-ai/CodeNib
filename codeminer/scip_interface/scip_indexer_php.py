...
class SCIPPHPIndexer(SCIPIndexerBase):
    ...
    def _copy_ignore(self, directory: str, names: list[str]) -> set[str]:
        ignored = {".git", ".codeminer", "index.scip", "index.decoded", "graph.pkl"}
    ...
    def _composer_command(self) -> list[str] | None:
        if shutil.which("composer") is not None:
            return ["composer"]
        composer = os.environ.get("CODEMINER_PHP_COMPOSER")
        if composer:
            return [composer]
        tools_dir = os.environ.get("CODEMINER_SCIP_TOOLS_DIR")
        if tools_dir:
    ...
    def _scip_php_package(self) -> str:
        return os.environ.get("CODEMINER_PHP_SCIP_PACKAGE", _DEFAULT_SCIP_PHP_PACKAGE)

    def _php_docker_command_for_root(self, root: Path, *args: str) -> list[str]:
        image = os.environ.get("CODEMINER_PHP_COMPOSER_IMAGE", _DEFAULT_COMPOSER_IMAGE)
        return [
            "docker",
            "run",
            "--rm",
