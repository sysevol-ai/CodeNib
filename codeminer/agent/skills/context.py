...
``codeminer/ops/`` (``RetrieveContext``, ``RerankContext``, ``TransformContext``,
...
* :class:`ComposerContexts` — a typed aggregate for *multi-context* (``custom``)
  composer skills (``codeminer_context``, ``bm25_search``) that genuinely need
...
class ComposerContexts:
    ...
    e.g. ``codeminer_context`` and the names-mode of ``bm25_search``) need
    ...
    ``codeminer.compiler.skill_context._package_contexts``.
    """
