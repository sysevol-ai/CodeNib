...
For a public trial, use the [precomputed static export](github_pages.md#publish-an-existing-story-wiki).
It serves Wiki pages, citations and page maps without a model backend; its
“Use with your agent” links lead to local setup. The interactive server below
is an operator/local runtime: browsing an uncached page can generate prose,
and Ask uses its configured provider account. Serving a static preview does
not change that intentional local behavior.
