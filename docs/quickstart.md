...
The version command must report `codenib 0.2.4`. Exact version pins keep the
...
```bash
python -m pip install "codenib[semantic-remote]==0.2.4"
...
```bash
python -m pip install "codenib[graph]==0.2.4"
...
```bash
python -m pip install "codenib[agent,semantic]==0.2.4"
export OPENAI_API_KEY=...
...
```bash
python -m pip install "codenib[agent,semantic,vertex]==0.2.4"
...
```bash
python -m pip install "codenib[mcp,semantic]==0.2.4"
codenib index /path/to/repository
