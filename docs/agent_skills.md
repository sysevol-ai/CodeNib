...
the dataset/HuggingFace boundary.

All conversion happens in one module, `codeminer/agent/boundary.py`, with one
site per direction:
...
Do **not** add ad-hoc `+1`/`-1` in executors or rendering paths — route
through the boundary helpers so the offset lives in exactly one place.
