# Guardian Inner Loop — Blueprint v1
...
**Why it exists.** The outer blueprint says L3 "is implemented and stays
as-is" (§ scope line). That is no longer true. A post-implementation audit of
one real Pier cycle found five P0 defects, and the loop's output on that cycle
was an admissible-looking `probe:2:1` evidence reference generated by a
Bubblewrap sandbox failure. L3 is structurally an agent loop and operationally
broken.
...
**Convention.** `file:line` in backticks is *current* code under
`codeminer/guardian/`. Identifiers without a line number are proposed.
...
**`evidence_status`** — `valid | invalid | environment`. Whether the run produced
something L2 may grade on. Guardian-specific; nothing in the API layer means
this, which is exactly why it needs its own name.
...
4. **Only an admissible run emits `probe-valid:`.** L2 appends an evidence
   reference *conditioned on* `evidence_status`, and `GRADE_RULES` requires the
   `probe-valid:` prefix. (I-6)
...
class InvestigationRunResult:
    ...
    evidence_status: str       # valid | invalid | environment  ← L2 gates on THIS
    reasoning: str
...
`evidence_status` is **derived from `tool_calls`, not asserted**:

- `valid` iff a cited command result contains at least one `PASSED` or `FAILED`
  test status and satisfies §8's source/corroboration rules;
- `environment` iff the prelude was blocked, or at least one test command was
  attempted and every such attempt produced only environment failure, `ERROR`,
  or no parsed test status;
- `invalid` otherwise.
...
Seven, and the seventh requests the normal exit. `TOOLS` already exists at
`runner.py:457`; this is its revision. Same design rule as the outer loop: code
owns invariants, the model chooses the call sequence.

| Tool | Signature | Notes |
|---|---|---|
| `read_code` | `(path, start_line, end_line) → ToolResult` | **new.** Direct, cheap, deterministic. Its absence is half of I-3 |
| `retrieve_evidence` | `(query, top_k) → ToolResult` | records full provenance (path, span, score) (I-9) |
| `run_existing_test` | `(node_id) → ToolResult` | uses the discovered recipe (§5.2), never a hard-coded interpreter |
| `synthesize_test` | `(target_symbol, body) → ToolResult` | its `tool_call_id` is what later calls reference |
| `run_synthesized_test` | `(tool_call_id) → ToolResult` | `parent_tool_call_id` = the synthesize call |
| `corroborate` | `(tool_call_id, method="fix"\|"differential") → ToolResult` | `parent_tool_call_id` = the run call. Replaces two tools whose results were joined by recency (I-7) |
| `submit_verdict` | `(verdict, reasoning, cites=[tool_call_id,…]) → ToolResult, then exit` | **the only normal exit.** It must be the turn's only tool call; the accepted call is recorded before `VerdictSubmitted`; `cites` must be non-empty for `confirmed`/`refuted` |

**Hard-coded, and each an invariant not a decision:** budget reservation (§6),
the deterministic prelude (§5.2), report parsing (§3.2), transition
admissibility (§8.2), per-call checkpointing, and content bounding. Everything
else is the model's choice.
...
```
1. environment_health()  python present? writable tmp? snapshot materialized?
2. discover_recipe()     pyproject.toml → tox.ini → Makefile → CI workflow
                         → poetry/uv lock → recipes validated in prior cycles
...
- If prelude operation 1 or 3 fails, L3 returns immediately with
  `verdict="inconclusive"`, `exit_status="environment_unavailable"`,
  `evidence_status="environment"` — **zero model calls spent.** A cycle that
  cannot run tests should cost nothing and say so plainly.
...
```python
prefix = {"valid": "probe-valid", "invalid": "probe-invalid", "environment": "env"}
...
```python
def _has_valid_probe(h):
    return any(e.startswith("probe-valid:") for e in h.evidence)
...
Three prefixes, not one, because the distinction the report must preserve is *we
probed and learned nothing* vs. *we could not probe*. Both are retained as
evidence — an `env:` reference is what tells the next cycle the environment was
broken here — but only `probe-valid:` is grade-admissible. The stronger form,
deferred to step 5 of §9, stores typed evidence objects and has `GRADE_RULES`
read the status directly rather than parsing a string prefix.

### 8.2 Corroboration as a named transition
...
Eight work items, in the audit's repair order. Each has a done-when test.

| # | Work item | Files | Done when |
|---|---|---|---|
| 1 | `submit_verdict` replaces forced finalization; normalize `rejected` → `refuted` | `investigator/runner.py`, `llm/codex_cli_chat.py` | no L3 model call is ever made with empty `tools`; budget exhaustion returns a typed result with zero further calls; L3 and L2 use the same negative-verdict spelling |
...
| 3 | `InvestigationTask` + `read_code` | `loop/runtime.py`, `runner.py` | L3 receives consequence, remedy, full locus, L2's excerpts and the commit diff; a behavioural verdict without a source span is rejected |
| 4 | Prelude: health check + recipe discovery | new `investigator/environment.py` | a repo with no runnable pytest yields `environment_unavailable` at zero token cost; a validated recipe survives into the next cycle |
| 5 | Conditional evidence references | `loop/runtime.py`, `loop/state.py` | an environment-failed investigation cannot produce a hypothesis at `grade == "finding"` |
| 6 | Transitions bound by `parent_tool_call_id` | `probes.py`, `runner.py` | a fix probe on test A cannot corroborate a failure of test B; an existing failing test corroborates even when a synthesized test exists |
...
Items 1 and 2 are independent. Item 5 depends on 2 (it gates on the derived
`evidence_status`). Item 6 depends on 2 and 3. Everything else is sequential.

**Item 1 before everything** for the same reason the outer plan puts the ontology
split first: while a tool-free call can happen, every other measurement is
contaminated by a nested agent doing something Guardian did not ask for.
...
4. only `probe-valid:` references satisfy `GRADE_RULES["finding"]`;
...
> One non-trivial post-commit cycle reads source, runs a real test inside the
> disposable snapshot, parses its report, stays within its grant, and produces
> either admissible evidence or a correctly classified environment failure — with
> no Bubblewrap error and no fake `probe:` evidence.
...
on test outcome (§3.2); `submit_verdict` as the only normal exit (§4, §7);
reservation-based budgeting (§6); three-prefix conditional evidence (§8.1);
corroboration as a transition bound by `parent_tool_call_id` (§8.2). Disposable
snapshots stay; older notes claiming L3 writes to production are stale.
...
per run, which is all §8.2's parentage needs; **cross-cycle** probe
...
Python/pytest: §5.2's recipe discovery and §3.2's report parser are the two seams
where a language registry attaches, with SWE-bench's per-repository parsers as
the precedent; only pytest is specified here.

**Check first if picking this up cold.** `runner.py:1011` (the forced call),
`probes.py:308-317` (the exit-code-to-FAIL mapping), `loop/runtime.py:219` (the
unconditional reference), and `codex_cli_chat.py:251` (the missing instruction on
the no-tools branch). Those four lines are the audit's failing cycle in full.
