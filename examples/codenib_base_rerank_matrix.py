...
"""
CodeNib-base × embedding-rerank matrix sweep.
...
tooling keeps working. ``<pair>`` below is ``<small>__x__<large>``:
  - results: ``<results>/rerank_codenib_base_<split>_<pair>.json``
  - profile: ``<profile>/codenib_base__dense_retrieve_plus_``
    ``embedding_rerank__<small>__<pair>.json``

Usage:
    python examples/codenib_base_rerank_matrix.py \\
...
from codenib.ops.rerank import rerank_by_embedding
...
def parse_args():
    p = argparse.ArgumentParser(
        description="CodeNib-base × embedding-rerank matrix sweep.",
...
def run_sweep(args):
    ...
    # codenib-base bakes GT into the dataset, so eval_path can be empty.
        ...
        try:
            ...
            for l_model, l_dim in large_specs:
                ...
                result_file = results_dir / (
                    f"rerank_{strategy_tag}_codenib_base_"
